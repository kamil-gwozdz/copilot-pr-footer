...
// Map a GraphQL StatusState rollup enum to our compact ci state.
function rollupToCi(state) {
...
function fetchAndCache(urls) {
  ...
  try {
    ...
    if (!installed || !authed || list.length === 0) { atomicWrite(CACHE, cache); return; }

    // 2) one aliased GraphQL query for every PR (a0, a1, …)
    const parsed = list.map(parsePrUrl);
    const frag = "fragment S on PullRequest { state isDraft reviewDecision commits(last:1){nodes{commit{statusCheckRollup{state}}}} }";
    const parts = [];
    parsed.forEach((p, i) => {
    ...
    list.forEach((url, i) => {
      ...
      if (pr) {
        cache[url] = {
          ...
          ci: rollupToCi(rollupStateOf(pr)),
          error: null,
          fetched_at: nowSec,
        };
      } else {
...
module.exports = {
  ...
  loadConfig, saveConfig, CACHE, CACHE_DIR, CONFIG, rollupToCi, parsePrUrl,
};
