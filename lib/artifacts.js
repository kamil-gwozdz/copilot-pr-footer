...
const UPDATE_RE =
  /^(?:\w+=\S+\s+)*gh\s+pr\s+(?:edit|comment|review|merge|ready|close|reopen|lock|unlock)\b|^(?:\w+=\S+\s+)*gh\s+issue\s+(?:edit|comment|close|reopen|lock|unlock|pin|unpin|transfer)\b/i;
...
const CANON = {
  ...
  gist: /https:\/\/gist\.github\.com\/[\w./-]+/g,
